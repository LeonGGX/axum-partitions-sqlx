...
pub async fn create_person_hdl(
    ...
    Ok(person_response(&mut flash, message))
}
...
pub async fn update_person_hdl(
    ...
    let message= format!("Musicien modifié avec succès : {:?}", person).to_owned();
...
pub async fn delete_person_hdl(
    ...
    let message= format!("Musicien effacé avec succès : {}", del).to_owned();
